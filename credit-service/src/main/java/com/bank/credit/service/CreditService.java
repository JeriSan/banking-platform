...
public class CreditService {
    ...
    private Credit init(Credit c){
        ...
        c.setStartDate(c.getStartDate()==null? LocalDate.now(): c.getStartDate());
    ...
    public Mono<Credit> pay(String creditId, BigDecimal amount){
        ...
        return repo.findById(creditId).flatMap(cr -> {
            ...
            CreditPayment p = CreditPayment.builder()
                    ...
                    .creditId(cr.getId()).timestamp(LocalDateTime.now()).amount(amount).balanceAfter(cr.getBalance()).build();
